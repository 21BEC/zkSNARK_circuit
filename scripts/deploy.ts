...
const BASE_PATH = "./circuits/Circuitfinal/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/Circuitfinal.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CircuitfinalVerifier.sol:Verifier");
